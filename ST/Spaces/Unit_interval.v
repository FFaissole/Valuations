...
Definition compact (U : OS U01) := forall (W : nat -> Q*Q),
             (forall x, U x -> exists n, op_rat (fst (W n)) (snd (W n)) x) 
             -> 
             (exists N, forall x, U x -> exists n, n <= N /\ 
...
Definition closed_interval (p q : Q) : OS U01.
Proof.
Admitted.
...
Definition open_interval_R (p q : Q) : OS cut.
Proof.
intros x. 
Admitted.

Definition closed_interval_R (p q : Q) : OS cut.
Proof.
Admitted.

Definition OU01_OR : OS U01 -> OS cut.
Proof.
intros U x. 
refine (semi_decide (CutZero <= x)).
SearchAbout SemiDecide.
simpl.
refine (semi_decide (hor (CutZero <= x) (x <= CutOne))).
...
Notation "] p , q [" := (open_interval p q) (at level 50).  
Notation "[ p , q ]" := (closed_interval p q) (at level 55). 
...
Lemma Q2enum : Enumerable (prod Q Q).
Admitted.

Lemma WSO_nat : exists W : nat -> Q*Q, forall (U : OS U01) x, 
           U x -> exists n, (op_rat (fst (W n)) (snd (W n)) x).
Proof.
destruct Q2enum as (q2n,Hq2n).
exists q2n.
intros U x Hux.
destruct (WSO U x) as (p,(q,(H1,H2))); trivial.
specialize (Hq2n (p,q)).
Admitted.
