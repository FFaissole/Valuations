...
Lemma sum_n_moy_aux_mon (p : nat) f g : fle f g ->
      Rlle (sum_n_moy_aux p f) (sum_n_moy_aux p g).
Proof.
intros Hfg.
induction p.
+ simpl; unfold Rlle; auto. 
+ simpl.
  assert (H : Rlle (RlPlus (f (p.+1)%nat) (sum_n_moy_aux p f))
          (RlP_plus (f (S p)) 
                   (sum_n_moy_aux p g))).
  apply Rllepos_plus_le_preserving; exact IHp.
  assert (H' : Rlle (RlP_plus (f (S p)) (sum_n_moy_aux p g))
                    (RlPlus (g (p.+1)%nat) (sum_n_moy_aux p g))).
  rewrite (RlPPlus_comm _ (sum_n_moy_aux p g)).
  rewrite (RlPlus_comm _ (sum_n_moy_aux p g)).
...
+ exists (QRlow (qn (S n))).
  intros p Hp.
  simpl; unfold semi_decide, Rllt_semi_dec, decidable_semi_decide.
  destruct (dec (p < S n)).
  apply top_greatest.
  assert (Hpn : p < (S n)).
  apply lt_le_trans with Qzero; trivial.
...
+ intros q. simpl in *.
  ...
  apply IHn in E2.
  induction n.
  - simpl in E2; unfold semi_decide, Rllt_semi_dec, 
         decidable_semi_decide in E2.
    destruct (dec (a < Qone)). 
    destruct (dec (b < Qzero)).
  ...
  - simpl in E2; unfold semi_decide, 
         Rllt_semi_dec, decidable_semi_decide in *.
    destruct (dec (a < Qone)).
    destruct (dec (b < S n)).
    apply plus_lt_compat; trivial.
    apply not_bot in E2; case E2.
...
+ simpl in Hq. simpl.
  unfold semi_decide, Rllt_semi_dec, 
    decidable_semi_decide in *; simpl in *.
  clear H. 
...
+ clear IHn. simpl.
  ...
  destruct (dec ((naturals.naturals_to_semiring 
              nat Q (S n) * q)%mc < S n)).
  destruct (dec (q < Qone)); trivial.
  apply top_greatest.
  assert (Hf : q < Qone). 
  assert (Hu : 
...
+ intros f g.
  ...
  apply RlP_eq0; simpl.
  transitivity (pred_multQ (elt (RlP_plus (sum_n_moy_aux N f)  
                                 (sum_n_moy_aux N g))) N).
  ...
  - apply imply_le.
    ...
    assert (Hn : (pos N / pos N) = 1%mc).
    transitivity (1/1)%mc.
    apply equal_dec_quotients.
    apply not_le_ne.
    intros HF.
    apply le_iff_not_lt_flip in HF. 
    assert (Hp : Qzero < pos N).
    apply (pos_is_pos).
    case (HF Hp).
    generalize rational_1_neq_0.
    apply apartness.apart_ne.
    ...
    assert (H1 : (Qmult (pos N) (pos N))
                / (Qmult (pos N) (pos N)) = 1%mc).
    transitivity (1/1)%mc.
    apply equal_dec_quotients.
    apply not_le_ne.
    intros HF.
    apply le_iff_not_lt_flip in HF. 
    assert (Hp : Qzero < Qmult (pos N) (pos N)).
