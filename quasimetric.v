...
Class QuasiMetric@{i j} (A:Type@{i}) {Aclose : Closeness A} :=
...
Global Instance quasi_hset@{i j} `{Funext}
  {A:Type@{i} } `{QuasiMetric@{i j} A} : IsHSet A.
Proof.
...
Lemma prod_premetric@{j} `{!QuasiMetric@{UA j} A} `{!QuasiMetric@{UB j} B}
  : QuasiMetric@{i j} (A /\ B).
Proof.
split;try apply _.
Qed.
...
Section close_arrow.
Context {A:Type} `{Bclose : Closeness B} `{!QuasiMetric B}.
...
Global Instance close_arrow_premetric : QuasiMetric (A -> B).
Proof.
split.
- apply _.
...
Lemma Interval_premetric@{i} `{!QuasiMetric@{UA i} A} a b
  : QuasiMetric@{UA i} (Interval a b).
Proof.
split.
...
Global Instance Q_quasimetric@{} : QuasiMetric Q.
Proof.
split;try apply _.
...
apply triangular with (x (e / 2 / 2)). unfold IsLimit in *. [Symmetry;apply E1|apply E2].
Qed.
...
Context {A:Type} {Aclose : Closeness A} `{!QuasiMetric A}
  `{Bclose : Closeness B} `{!QuasiMetric B} {Blim : Lim B}
   `{!CauchyComplete B}.
(*
