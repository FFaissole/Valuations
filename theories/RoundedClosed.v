...
- intros a b c E E';revert E; apply (Trunc_ind _);intros [q [E1 E2]];
  revert E';apply (Trunc_ind _);intros [r [E3 E4]].
  apply tr;exists q. split.
  + apply RC_le with Qle r. red; auto with qarith.
...
Proof.
intros a b;apply RC_eq; auto; simpl;intros q;split;intros E;
...
- apply pred_plus_pr in E;revert E;apply (Trunc_ind _);intros [r [s [E1 [E2 E3]]]].
  apply semi_decidable in E1. rewrite E3. apply RC_le with Qle s;trivial.
  red; auto with qarith. apply le_or_lt. 
...
Proof.
intros a b.
generalize ((@orders.le_not_lt_flip Rlow) Rlle Rlle Rllt). 
...
- intros a b;split.
  + intros E. apply (antisymmetry le);apply rlow_not_lt_le_flip;intros E1;
    apply E;unfold apart;simpl;unfold RCApart;auto.
...
generalize Rlow_fullpseudoorder. 
intros (HN1,(HN21,HN22,HN23,HN24,HN25), HN3).
apply HN3. 
intros SH. 
assert (Hk : c < b < c); auto. 
case ((HN23 c b) Hk). 
red in HS. unfold Rllt in HS.
unfold RCLt_l in HS. 
revert HS;apply (Trunc_ind _); intros [r [E1 E2]].
red in E, HS'.  
unfold Rlle, RCLe_l in E, HS'. 
assert (EH : forall q, elt Q Qlt (a + b) q <-> elt Q Qlt (a + c) q). 
split. apply E. apply HS'. 
clear E HS'. 
apply E2. 
Admitted. 
...
Lemma rlowp_plus_le_reflecting : forall a : RlowPos, OrderReflecting (a +).
Proof.
  intros a b c E.
  (*
...
Definition QMult_RlP : Q+ -> RlowPos -> RlowPos.
Admitted.
